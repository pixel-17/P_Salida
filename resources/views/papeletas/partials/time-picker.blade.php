{{--
    Selector de hora en formato 12h con AM/PM explícito.

    Reemplaza al <input type="time"> nativo: ese input depende del idioma
    / configuración regional del navegador para decidir si muestra 12h o
    24h, y su atributo `min` fue la causa de un bug donde, de noche, se
    bloqueaba elegir una hora normal del día siguiente (ver create.blade.php
    para el detalle del bug de zona horaria). Con selects propios el
    formato es siempre el mismo para todos los usuarios y no hay ningún
    `min` nativo que pueda bloquear una hora válida.
    ...
    campo. Los tres selects visibles NO tienen `name` — son solo UI.
...
<div id="picker-{{ $id }}" class="grid grid-cols-3 gap-1.5">
